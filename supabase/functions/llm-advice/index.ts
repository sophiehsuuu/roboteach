...
async function handleBlockAnalysis(req: Request, headers: any): Promise<Response> {
  try {
    ...
    // Special handling for natural language generation - completely separate prompt
    const prompt = pickedSymptom === 'natural-language-generation' ? `
【積木程式生成器】
...
【積木程式生成器】
學生用自然語言描述想要的機器人行為，請為他們生成全新的積木程式指令。

**重要：這是新程式生成，不是現有程式分析！**

**學生需求**：請仔細閱讀學生的自然語言描述，然後生成對應的積木程式。
...
**格式要求**：用繁體中文，提供具體、可執行的積木指令，不要分析現有程式。
...
❌ 不完整描述：
- "機器人前進，然後停止。感應器偵測紅色" (缺少條件關係)
- "馬達A、B運作，感應器檢測距離" (缺少動作順序)
- "程式啟動時機器人行動" (過於抽象，缺少具體邏輯)

**根據上方【程式邏輯流程結構】和【增強型積木檢測】進行分析**，用繁體中文回覆，不超過50字，**必須描述完整的執行流程和條件邏輯關係**。

**關鍵提醒：**
- 如果同時看到感應器積木(如"Sensor A: is color red")和控制積木(如"if...then")，說明條件邏輯已完整存在
- 不要建議添加已經存在的功能
- 必須用條件句式描述現有的感應器-條件-動作關係

格式要求：啟動動作→條件判斷→分支結果，使用"當...時"或"如果...則...否則..."的句式。
` : `
【診斷要求】
請根據上述積木設定，針對學生描述的問題進行分析：
1. 檢查馬達設定：端口、速度、方向是否正確
2. 檢查感應器設定：類型、參數、條件是否合適
3. **重點分析條件邏輯：檢查if/then/else結構中感應器如何控制馬達行為**
4. 檢查程式結構：是否有開始、循環、停止等控制積木
5. **檢查移動控制：包括條件移動（if...then）、停止指令（stop moving）、方向控制等**
6. **分析因果關係：感應器觸發→條件判斷→馬達動作的完整流程**
7. 如果發現參數問題（如速度太低、方向錯誤），請具體指出並建議修正
8. 如果缺少必要積木，請建議添加
...
- **條件邏輯說明：用條件句式描述感應器和馬達的關係（"當...時"，"如果...則"）**
- **避免分離描述：不要將感應器和馬達行為分開說明，要顯示因果關係**
- 如缺少關鍵積木，請直接提示對方補上
